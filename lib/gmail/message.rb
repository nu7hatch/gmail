module Gmail
  class Message
    ...
    # Move to trash / bin.
    def delete!
      # Support different trashes name
      trashes = %w([Gmail]/Corbeille [Gmail]/Bin [Gmail]/Trash)
      trash = nil
      trashes.each do |t|
        trash = t if @gmail.labels.exist? t
      end
      unless trash.nil? || trashes.append('[Gmail]/Spam').include?(@mailbox.name)
